...
function ScriptableToolKit(t,e,i){return new class{constructor(t,e,i){this.local=FileManager.local();this.icloud=FileManager.iCloud();this.curDateCache=this.local.joinPath(this.local.documentsDirectory(),"curDateCache");this.options=i;this.tgEscapeCharMapping={"&":"＆"};this.userAgent=`Mozilla/5.0 (Macintosh; Intel Mac OS X 10_13_6) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/12.0.2 Safari/605.1.15`;this.prefix=`lk`;this.name=t;this.id=e;this.data=null;this.dataFile=`${this.prefix}${this.id}.json`;this.bgImgPath=`${this.prefix}${this.id}Bg.jpg`;this.bgImgPath=this.local.joinPath(this.local.documentsDirectory(),this.bgImgPath);this.lang=Device.language();this.msg={zh:["在开始之前，先进入主屏幕，进入图标排列模式。滑到最右边的空白页，并进行截图。","看起来你选择的图片不是iPhone的截图，或者你的iPhone不支持。请换一张图片再试一次。","你想创建什么尺寸的widget？","你想把widget放在哪里？"," (请注意，您的设备只支持两行小部件，所以中间和底部的选项是一样的)。","widget的背景图已裁切完成，想在Scriptable内部使用还是导出到相册？","已经截图，继续","退出去截图","小","中","大","顶部左边","顶部右边","中间左边","中间右边","底部左边","底部右边","顶部","中间","底部","在Scriptable内部使用","导出到相册","填写遮罩层颜色。（格式：#000000）","颜色（格式：#000000）","填写遮罩层不透明度（0-1之间）","0-1之间","确定","取消"],en:["Before you start, go to your home screen and enter wiggle mode. Scroll to the empty page on the far right and take a screenshot.","It looks like you selected an image that isn't an iPhone screenshot, or your iPhone is not supported. Try again with a different image.","What size of widget are you creating?","What position will it be in?"," (Note that your device only supports two rows of widgets, so the middle and bottom options are the same.)","Your widget background is ready. Would you like to use it in a Scriptable widget or export the image?","Continue","Exit to Take Screenshot","Small","Medium","Large","Top left","Top right","Middle left","Middle right","Bottom left","Bottom right","Top","Middle","Bottom","Use in Scriptable","Export to Photos","Fill in the mask layer color. (Format: #000000)","Color.(Format: #000000)","Fill in the mask layer opacity (between 0-1)","between 0-1","Confirm","Cancel"]};this.curLang=this.msg[this.lang]||this.msg.en;this.isSaveLog=this.getResultByKey(`${this.prefix}IsSaveLog${this.id}`,true);this.isEnableLog=this.getResultByKey(`${this.prefix}IsEnableLog${this.id}`,true);this.logSeparator="\n██";this.now=(new Date).getTime();this.execStatus=true;this.notifyInfo=[]}getResultByKey(t,e){if(!this.options){return e}const i=this.options[t];if(this.isEmpty(i)){return e}else{return i}}appendNotifyInfo(t,e){if(e==1){this.notifyInfo=t}else{this.notifyInfo.push(t)}}prependNotifyInfo(t){this.notifyInfo.splice(0,0,t)}execFail(){this.execStatus=false}sleep(t){return new Promise(e=>setTimeout(e,t))}log(t){if(this.isEnableLog)console.log(`${this.logSeparator}${t}`)}logErr(t){this.execStatus=false;if(this.isEnableLog){console.log(`${this.logSeparator}${this.name}执行异常:`);console.log(t);console.log(`\n${t.message}`)}}getContainer(t){return t=="local"?this.local:this.icloud}async getVal(t,e,i){let r=this.getContainer(e);let a="";try{let t=r.joinPath(r.documentsDirectory(),this.dataFile);if(!r.fileExists(t)){return Promise.resolve(i)}a=await r.readString(t);a=JSON.parse(a)}catch(t){throw t}return Promise.resolve(a.hasOwnProperty(t)?a[t]:i)}async getDataFile(t){let e=this.getContainer(t);let i="";try{let t=e.joinPath(e.documentsDirectory(),this.dataFile);if(!e.fileExists(t)){return Promise.resolve("")}i=await e.readString(t)}catch(t){throw t}return Promise.resolve(i)}async setVal(t,e,i){let r=this.getContainer(i);let a;let s=r.joinPath(r.documentsDirectory(),this.dataFile);try{if(!r.fileExists(s)){a={}}else{a=await r.readString(s);a=JSON.parse(a)}}catch(t){a={}}a[t]=e;r.writeString(s,JSON.stringify(a))}async get(t,e=(()=>{})){let i=new Request("");i.url=t.url;i.method="GET";i.headers=t.headers;const r=await i.loadString();e(i.response,r);return r}async post(t,e=(()=>{})){let i=new Request("");i.url=t.url;i.body=t.body;i.method="POST";i.headers=t.headers;const r=await i.loadString();e(i.response,r);return r}async loadScript({scriptName:t,url:e}){this.log(`获取脚本【${t}】`);const i=await this.get({url:e});this.icloud.writeString(`${this.icloud.documentsDirectory()}/${t}.js`,i);this.log(`获取脚本【${t}】完成🎉`)}require({scriptName:t,url:e="",reload:i=false}){if(this.icloud.fileExists(this.icloud.joinPath(this.icloud.documentsDirectory(),`${t}.js`))&&!i){this.log(`引用脚本【${t}】`);return importModule(t)}else{this.loadScript({scriptName:t,url:e});this.log(`引用脚本【${t}】`);return importModule(t)}}async generateInputAlert(t,e,i){let r=[];let a=new Alert;a.message=t;a.addTextField(e,i);a.addCancelAction(this.curLang[27]);a.addAction(this.curLang[26]);r[0]=await a.presentAlert();r[1]=a.textFieldValue(0);return r}async generateAlert(t,e){let i=new Alert;i.message=t;for(const t of e){i.addAction(t)}return await i.presentAlert()}isEmpty(t){return typeof t=="undefined"||t==null||t==""||t=="null"}isWorkingDays(t){return new Promise(async(e,i)=>{const r=t.getMonth()+1>9?t.getMonth()+1:"0"+(t.getMonth()+1);const a=t.getDate()>9?t.getDate():"0"+t.getDate();const s=`${t.getFullYear()}${r}${a}`;let o=0;try{let t=await this.getVal("curDateCache","local","fff");if(s==t.split("-")[0]){o=t.split("-")[1];this.log("already request")}else{this.log("send request");const t={url:"http://tool.bitefu.net/jiari/?d="+s};await this.post(t,(t,e)=>{o=e;this.setVal("curDateCache",`${s+"-"+o}`,"local")})}}catch(t){this.logErr(t)}finally{e(o==0?workingDaysFlag:holidayFlag)}})}randomString(t){t=t||32;var e="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz1234567890";var i=e.length;var r="";for(let a=0;a<t;a++){r+=e.charAt(Math.floor(Math.random()*i))}return r}autoComplete(t,e,i,r,a,s,o,l,n,h){t+=``;if(t.length<a){while(t.length<a){if(s==0){t+=r}else{t=r+t}}}if(o){let e=``;for(var g=0;g<l;g++){e+=h}t=t.substring(0,n)+e+t.substring(l+n)}t=e+t+i;return this.toDBC(t)}customReplace(t,e,i,r){try{if(this.isEmpty(i)){i="#{"}if(this.isEmpty(r)){r="}"}for(let a in e){t=t.replace(`${i}${a}${r}`,e[a])}}catch(t){this.logErr(t)}return t}toDBC(t){var e="";for(var i=0;i<t.length;i++){if(t.charCodeAt(i)==32){e=e+String.fromCharCode(12288)}else if(t.charCodeAt(i)<127){e=e+String.fromCharCode(t.charCodeAt(i)+65248)}}return e}getWidgetBg(){return this.local.readImage(this.bgImgPath)}phoneSizes(){return{2688:{small:507,medium:1080,large:1137,left:81,right:654,top:228,middle:858,bottom:1488},1792:{small:338,medium:720,large:758,left:54,right:436,top:160,middle:580,bottom:1e3},2436:{small:465,medium:987,large:1035,left:69,right:591,top:213,middle:783,bottom:1353},2532:{small:474,medium:1014,large:1062,left:78,right:618,top:231,middle:819,bottom:1407},2208:{small:471,medium:1044,large:1071,left:99,right:672,top:114,middle:696,bottom:1278},1334:{small:296,medium:642,large:648,left:54,right:400,top:60,middle:412,bottom:764},1136:{small:282,medium:584,large:622,left:30,right:332,top:59,middle:399,bottom:399},1624:{small:310,medium:658,large:690,left:46,right:394,top:142,middle:522,bottom:902}}}remove(t){this.local.remove(t)}cropImage(t,e,i,r){let a=new DrawContext;a.size=new Size(e.width,e.height);a.drawImageAtPoint(t,new Point(-e.x,-e.y));a.setFillColor(new Color(i,Number(r)));a.fillRect(new Rect(0,0,t.size["width"],t.size["height"]));return a.getImage()}async widgetCutBg(){var t;t=this.curLang[0];let e=[this.curLang[6],this.curLang[7]];let i=await this.generateAlert(t,e);if(i)return;let r=await Photos.fromLibrary();let a=r.size.height;let s=this.phoneSizes()[a];if(!s){t=this.curLang[1];await this.generateAlert(t,["OK"]);return}t=this.curLang[2];let o=[this.curLang[8],this.curLang[9],this.curLang[10]];let l=await this.generateAlert(t,o);t=this.curLang[3];t+=a==1136?this.curLang[4]:"";let n={w:"",h:"",x:"",y:""};if(l==0){n.w=s.small;n.h=s.small;let e=["Top left","Top right","Middle left","Middle right","Bottom left","Bottom right"];let i=[this.curLang[11],this.curLang[12],this.curLang[13],this.curLang[14],this.curLang[15],this.curLang[16]];let r=await this.generateAlert(t,i);let a=e[r].toLowerCase().split(" ");n.y=s[a[0]];n.x=s[a[1]]}else if(l==1){n.w=s.medium;n.h=s.small;n.x=s.left;let e=["Top","Middle","Bottom"];let i=[this.curLang[17],this.curLang[18],this.curLang[19]];let r=await this.generateAlert(t,i);let a=e[r].toLowerCase();n.y=s[a]}else if(l==2){n.w=s.medium;n.h=s.large;n.x=s.left;let e=[this.curLang[17],this.curLang[19]];let i=await this.generateAlert(t,e);n.y=i?s.middle:s.top}let h=await this.generateInputAlert(this.curLang[22],this.curLang[23],"#000000");if(h[0]==-1)return;let g=await this.generateInputAlert(this.curLang[24],this.curLang[25],"0.2");if(g[0]==-1)return;let c=this.cropImage(r,new Rect(n.x,n.y,n.w,n.h),h[1],g[1]);t=this.curLang[5];const u=[this.curLang[20],this.curLang[21]];const d=await this.generateAlert(t,u);if(d){Photos.save(c)}else{this.local.writeImage(this.bgImgPath,c)}Script.complete()}}(t,e,i)}
