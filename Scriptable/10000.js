...
async function createWidget(w, pretitle, subt, flowRes, voiceRes) {
    ...
    for (let n = 0; n < infoOrder.length; n++) {
        ...
        if ("话费" === type) {
            ...
            let minText = w.addText(`${$.execStatus?'':'⚬'}更新于：${hours > 9 ? hours : "0" + hours}:${minutes > 9 ? minutes : "0" + minutes}`)
            minText.font = currentSpec.fontStyle
            minText.textColor = currentColor
            minText.rightAlignText()
        }
